    ...
    <script type="text/javascript">
        ...
        function drawCustomerChart() {
            var data = google.visualization.arrayToDataTable([
                ['Gender', 'Count'],
                <c:forEach var="entry" items="${customerGender.entrySet()}">
        ...
        function drawDeliveryChart() {
            var data = google.visualization.arrayToDataTable([
                ['Gender', 'Count'],
                <c:forEach var="entry" items="${deliveryGender.entrySet()}">
                    ['${entry.key}', ${entry.value}],
                </c:forEach>
            ]);

            var options = {
                title: 'Delivery Staff Gender Distribution',
                pieHole: 0.4,
                chartArea: {width: '80%', height: '80%'}
            };
        ...
        function drawManagingChart() {
            var data = google.visualization.arrayToDataTable([
                ['Gender', 'Count'],
                <c:forEach var="entry" items="${managingGender.entrySet()}">
                    ['${entry.key}', ${entry.value}],
                </c:forEach>
            ]);

            var options = {
                title: 'Managing Staff Gender Distribution',
                pieHole: 0.4,
                chartArea: {width: '80%', height: '80%'}
            };
